...
npts=sachead%npts
!write(*,*)'npts=',npts
open(funit,file=trim(fname),form='unformatted',access='direct',recl=158*4+npts*4,iostat=iso)
if(iso/=0)then
   write(*,*)"Error in openning file ",trim(fname)
...
implicit none
character*(*) fname
integer:: nmax,npts,nerr,num,i,iso
...
sachead%user0=real(num)
sachead%b=-int(npts/2)*sachead%delta
sachead%e= int(npts/2)*sachead%delta
sachead%kevnm=trim(sachead1%kstnm)
sachead%kstnm=trim(sachead2%kstnm)
sachead%o=0
...
if(nerr.eq.-1)write(*,*) 'Something wrong with ncf sac writing for file: ',trim(fname)
end subroutine write_ncf_sac

subroutine write_ext_sac(fname,array,sachead,npts,nzyear,nzjday,nzhour,nzmin,nzsec,nzmsec,kho,nerr)
use sacio
implicit none
type(sac_head):: sachead
integer,parameter :: nmax=4000000
...
real:: array(nmax)
sachead%npts  =npts
sachead%nzyear=nzyear
sachead%nzjday=nzjday
sachead%nzhour=nzhour
sachead%nzmin =nzmin
sachead%nzsec =nzsec
sachead%nzmsec=nzmsec
sachead%khole =kho
call write_sac(fname,array,sachead,nerr)
if(nerr.eq.-1)write(*,*) 'Something wrong with ncf sac writing for file: ',trim(fname)
end subroutine write_ext_sac
